 ...
 */
export function moderationState(reportedAt: string[], now: Date = new Date()): ModerationState {
  const cycles = Math.floor(reportedAt.length / REPORTS_PER_CYCLE);

  if (cycles === 0) {
    return { cycles: 0, permanent: false, hidden: false };
  }

  if (cycles >= 2) {
    return { cycles, permanent: true, hidden: true };
  }

  // Le masquage court à partir du signalement qui a bouclé le cycle.
  const trigger = reportedAt[REPORTS_PER_CYCLE - 1]!;
